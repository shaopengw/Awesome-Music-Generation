...
from MMGen_train.modules.clmp.open_clip import create_model
from MMGen_train.modules.clmp.training.data import get_audio_features
...
from MMGen_train.modules.audiomae.AudioMAE import Vanilla_AudioMAE
from MMGen_train.modules.phoneme_encoder.encoder import TextEncoder
...
from MMGen_train.modules.audiomae.sequence_gen.model import CLAP2AudioMAE
from MMGen_train.modules.audiomae.sequence_gen.sequence_input import (
    Sequence2AudioMAE,
)
import numpy as np
from MMGen_train.modules.audiomae.sequence_gen.model import Prenet
...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.embed_mode == "audio":
            if not self.training:
                print("INFO: clmp model calculate the audio embedding as condition")
            with torch.no_grad():
                # assert (
                ...
                # save_dir = "/work/home/acdfqcivpi/AudioLDM-training-finetuning-main_all_run/audio_emded"
                
                
                # save_path = os.path.join(save_dir, f"mel_data_epoch_batch_2.pt")
                # torch.save(mel.detach().cpu(), save_path)
                # import sys 
                # sys.exit()
                ...
                # save_path = os.path.join(save_dir, f"audio_dict_data_epoch_batch_2.pt")
                # torch.save(audio_dict["mel_fusion"].detach().cpu(), save_path)
                # print("audio_dict:::::::::" ,audio_dict)
                # [bs, 512]
                embed = self.model.get_audio_embedding(audio_dict)
                # save_path = os.path.join(save_dir, f"embed_data_epoch_batch_2.pt")
                # torch.save(embed.detach().cpu(), save_path)
        ...
        # pre_process_embed = embed.detach().clone()
        # pre_save_path = os.path.join(save_dir, f"pre_embed_epoch_batch2.pt")
        # torch.save(pre_process_embed.cpu(), pre_save_path)
        # print(f"Saved pre-process embeddings at {pre_save_path}")
        ...
        # post_save_path = os.path.join(save_dir, f"post_embed_epoch_batch2.pt")
        # torch.save(embed.detach().cpu(), post_save_path)
        # print(f"Saved post-process embeddings at {post_save_path}")
