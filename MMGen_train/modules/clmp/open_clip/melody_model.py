...
class Melody(nn.Module):
    ...
    def forward(self, melody_texts, device):
        ...
        for melody_text in melody_texts:
            ...
            tokens = text_to_indexed_segments('/mnt/data/wmz/audioldm_train_faiss/audio-ldm-training-finetuning-main_all_run/audioldm_train/modules/clap/songcomposer/only_index/re_index.json', melody_text)
...
#                  tokenizer_path = '/mnt/data/wmz/audioldm_train_faiss/audio-ldm-training-finetuning-main_all_run/audioldm_train/modules/clap/songcomposer/melody_encoder'):
                 
#         super(Melody, self).__init__()
#         self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, trust_remote_code=True)
#         self.mlp_4096_to_1024 = nn.Linear(4096, 1024)
#         self.LayerNorm = nn.LayerNorm(4096, eps=1e-5)
#         self.pooler = MelodyPooler(hidden_size=4096)
...
#                 token_embeddings = token_embeddings[:target_shape[0], :]  # [400,128]
#             all_token_embeddings.append(token_embeddings) 
        

#         all_token_embeddings = torch.stack(all_token_embeddings) # [32,400,128]

#         all_token_embeddings = self.LayerNorm(all_token_embeddings)
#         all_token_embeddings = self.pooler(all_token_embeddings)
#         token_embeddings = self.mlp_4096_to_1024(all_token_embeddings)
#         return token_embeddings 